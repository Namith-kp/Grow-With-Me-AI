...
const Header: React.FC<HeaderProps> = ({ currentView, setView, userProfile, onLogout, pendingRequestCount }) => {
    ...
    if (!userProfile) {
        return (
            <header className="fixed top-0 left-0 right-0 bg-black/50 backdrop-blur-sm z-50 border-b border-neutral-800">
    ...
    return (
        <>
            {/* Mobile header */}
            <header className="lg:hidden fixed top-0 left-0 right-0 bg-black/70 backdrop-blur-sm z-50 border-b border-neutral-800 flex items-center h-16 px-4 justify-between">
            ...
            <nav className={`fixed bottom-0 left-0 right-0 z-50 bg-neutral-900 border-t border-neutral-800 flex justify-between items-center h-16 px-1 sm:hidden lg:hidden gap-1 transition-transform duration-300 ${showNav ? 'translate-y-0' : 'translate-y-full'}`}>
            ...
            { (sidebarOpen || sidebarVisible) && (
                <div className="fixed inset-0 z-50 lg:hidden">
                    {/* Overlay */}
                    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm transition-opacity duration-300 opacity-100 pointer-events-auto" onClick={() => setSidebarOpen(false)}></div>
                    {/* Sidebar with slide animation */}
                    <aside
                        className={`relative w-64 bg-neutral-900 text-white p-4 flex flex-col h-full border-r border-neutral-800 z-50 transform transition-transform duration-300 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} pointer-events-auto`}
            ...
            {/* Sidebar for desktop */}
            <aside className="hidden lg:flex w-64 bg-neutral-900 text-white p-4 flex-col h-screen border-r border-neutral-800 fixed">
